...
def residual_analysis(y, freq=1, fclim=[], show=False, ax=None):
    ...
    .. [3] http://nbviewer.ipython.org/github/demotu/BMC/blob/master/notebooks/ResidualAnalysis.ipynb
    ...
    if not len(fclim) or np.any(fclim < 0) or np.any(fclim > freq / 2):
        ...
        reslog = np.log(np.abs(res[np.int(fc1):np.int(fc2) + 1] - res[np.int(fc2)]) +
                        10 * np.finfo(np.float).eps)
        Blog, Alog = np.polyfit(freqs[np.int(fc1):np.int(fc2) + 1], reslog, 1)
    ...
    if len(fclim) and fclim[0] < fclim[1]:
        B, A = np.polyfit(freqs[np.int(fclim[0]):np.int(fclim[1])], res[np.int(fclim[0]):np.int(fclim[1])], 1)
...
def _plot(y, freq, freqs, res, fclim, fc_opt, B, A, ax):
    ...
    try:
        ...
        if fc_opt:
            ...
            ax[0].plot(freqs[np.int(fclim[0])], res[np.int(fclim[0])], 'r>',
                       freqs[np.int(fclim[1])], res[np.int(fclim[1])], 'r<', ms=9)
